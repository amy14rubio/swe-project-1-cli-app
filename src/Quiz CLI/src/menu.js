...
const makeFileIO = require('../utils/makeFileIO.js');
...
const runQuiz = async (quizInstance, timeLimit) => {
  ...
  while (questionCounter < quizInstance.answerChoices.length) {
    ...
    const timerPromise =
      timeLimit != null
        ? new Promise((resolve) => setTimeout(() => resolve('timeout'), timeLimit))
        : null;
  ...
  if (quizInstance.isTopFive() || !Highscores.highscore.length) {
...
const showMenu = async () => {
  ...
  // Load Highscores History
  const pathToHighscoresHistory = path.join(__dirname, '../data/highscores.json');
  const highscoresHistoryIO = makeFileIO(pathToHighscoresHistory);
  const loadedHighscoresHistory = highscoresHistoryIO.read();
  Highscores.setHighscoreHistory(loadedHighscoresHistory || []);
  ...
  while (isRunning === true) {
    ...
    if (menuChoice === '1') {
      ...
      timeLimit !== undefined ? await runQuiz(selectedQuiz, timeLimit) : console.clear();
      ...
      highscoresHistoryIO.write(Highscores.getHighscoreHistory());
